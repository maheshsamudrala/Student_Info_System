...
public class AssignCourseDao {
	...
	public String assignCourses_mainFunction(String facultyname,
			String course_names) {
		...
		try {
			...
			for (int i = 0; i < Splitted.length; i++) {
				String QueryToGetCourseId = "select course_id from course where course_name='"
						+ Splitted[i] + "'";
				stmt1 = con.createStatement();
				  ResultSet courseIdResultSet = stmt1
						.executeQuery(QueryToGetCourseId);
		...
		try {
			...
			String QueryToGetFacultyId = "select faculty_id from faculty where faculty_name='"
					+ facultyname + "'";
			stmt2 = con.createStatement();
			ResultSet FacultyIDResultSet = stmt2
					.executeQuery(QueryToGetFacultyId);
			if (FacultyIDResultSet.next()) {

				//System.out.println(FacultyIDResultSet.getString("faculty_id"));
				//System.out.println(CourseId.get(2));
